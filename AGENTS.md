...
本地：`pnpm build && pnpm pack && pnpm exec playwright install chromium && pnpm test:mount`。CI 钉 `@deepseek-ai/dsh@0.1.5-alpha.1`（`alpha` dist-tag；peer 下限 `^0.1.5-alpha.1`）。e2e spec 命名 `*.e2e.ts` + vitest `exclude` 双保险；**改 `exclude` 必须保留默认排除项**（exclude 整体替换默认值）。

---

## 3. DSH 0.1.5 适配要点（0.1.5-alpha.1+ 基线）

**v0.19.0-alpha.0 起仅支持 DSH 0.1.5-alpha.1+**（peer 下限 `^0.1.5-alpha.1`，CI 钉 `@deepseek-ai/dsh@0.1.5-alpha.1`，DSH alpha 线发在 `alpha` dist-tag）。0.1.2-rc.1 稳定线用户继续用 v0.18.x（npm `latest`）——0.1.5 的会话事件模型与文件打开漏斗都变了，插件不再对 0.1.2 做运行时兼容。发版：release.yml 按版本号是否含 `-` 自动选 `alpha`/`latest` dist-tag。
...
10. **聊天文件打开漏斗已改为原生右侧栏**：0.1.5 客户端不再有任何 `remote.session.openWorkspacePath` 调用——聊天里一切文件打开（工具行 / 产物行 / 正文提及 / 行内代码路径）统一走 `ctx.sidebarRight.openResource(fileAddressFor(sessionId, cwd, path))`（`packages/client/ui-chat/src/client/apply.ts` 是唯一调用点）。插件此前的 `openpath-intercept`（defineProperty 遮蔽 `remote.session` 方法）随之删除：0.1.5 上它是死代码，且会劫持宿主的「在外部应用打开」语义。文件当前落到原生栏的内置 `text` 预览；阶段二由插件注册 `dsh-resource://file/**` 类型（默认 `extension` 带，压过 `text` 的 `fallback`）接管。
