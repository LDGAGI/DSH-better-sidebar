...
describe('side card preferences', () => {
  ...
  it('defaults fileIconTheme to builtin; only a stored string survives', async () => {
    // The colored theme is opt-in: absent or malformed keeps the monochrome
    // built-in glyphs (and never fetches the icon chunk).
    expect((await loadPrefs(wire({}))).fileIconTheme).toBe('builtin')
    expect((await loadPrefs(wire({ fileIconTheme: 7 }))).fileIconTheme).toBe('builtin')
    expect((await loadPrefs(wire({ fileIconTheme: 'colored' }))).fileIconTheme).toBe('colored')
    // An unknown id is preserved here and treated as builtin by the loader.
    expect((await loadPrefs(wire({ fileIconTheme: 'ghost' }))).fileIconTheme).toBe('ghost')
  })
  ...
  it('skips the default seed tab when the editor (files window) type is disabled', () => {
    ...
    for (const editorExplorer of [true, false]) {
      const openStore = createSidebarStore()
      openStore.setPrefs({ openByDefault: true, defaultWidthPercent: 30, autoOpenSubagent: true, autoOpenJobs: true, agentTerminalTools: false, agentOpenTools: false, bottomPanelAutoTerminal: true, terminalFontFamily: '', terminalFontSize: 13, interceptOpenPath: true, editorExplorer, changesDiffFloat: true, workspaceFence: true, fileIconTheme: 'builtin', terminalShell: '', terminalShellArgs: '', titleBarScheme: 'auto', titleBarPresetId: '', customCss: '', titleBarCompat: false, titleBarStripPx: 40, htmlViewerNoSandbox: false, htmlViewerDefaultUnsafe: false, browserNoSandbox: false, browserInterceptLinks: true, browserInterceptHttp: true, browserInterceptHttps: false, browserAllowedLoopback: "", tabsEnabled: {}, viewersEnabled: {}, pluginSettings: {} })
