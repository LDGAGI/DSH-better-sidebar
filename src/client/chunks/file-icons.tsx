/**
 * Lazy chunk entry: the colored file-icon theme dataset — brand and generic
 * glyphs paired with their display color (extensions / exact file names /
 * directory names), consumed by `registerColoredFileIcons` in the core half
 * when the user turns the theme on.
 *
 * Data provenance: the tables below come from PR #429 (@fenter,
 * `feat/file-type-icons`, the 563-entry table); they are reused here as the
 * plugin's built-in OPTIONAL icon theme. The case-variant folder keys
 * (`Android`/`android`, `IOS`/`ios`) were folded into one rule because the
 * resolver matches names case-insensitively.
 *
 * Why a chunk: ~160 react-icons glyphs (Simple Icons brands + VSCodicons) and
 * ~700 lines of table would otherwise sit in the startup bundle for a theme
 * most users never enable. The core half only fetches this on demand.
 *
 * Skin contract: this module is the ONE documented exception to the
 * token-only rule (guide §12) — brand colors are identity, not chrome, and
 * the theme is opt-in, so they cannot ride `--dsw-alias-*`. Nothing here is
...
import {
  ...
  SiCrystal, SiCss, SiCypress, SiDart, SiDjango, SiDocker, SiDocusaurus,
  SiDotenv, SiDotnet, SiElixir, SiElm, SiErlang, SiEsbuild, SiEslint,
  SiGatsby, SiGit, SiGithub, SiGitlab, SiGnubash, SiGo, SiGodotengine,
  SiGooglecloud, SiGradle, SiGraphql, SiGrunt, SiGulp, SiHaskell, SiHelm,
  SiHtml5, SiJavascript, SiJenkins, SiJest, SiJulia, SiKotlin, SiKubernetes,
  SiLerna, SiLess, SiLua, SiMarkdown, SiMysql, SiNetlify, SiNextdotjs, SiNim,
  SiNixos, SiNodedotjs, SiNpm, SiNuxt, SiNx, SiOcaml, SiPerl, SiPhp, SiPnpm,
  SiPostcss, SiPrettier, SiPrisma, SiPython, SiRacket, SiReact, SiRemix,
  SiRollupdotjs, SiRuby, SiRust, SiSass, SiScala, SiSolidity, SiSqlite,
  SiStorybook, SiStylelint, SiSvelte, SiSwc, SiSwift, SiTailwindcss, SiTauri,
  SiTerraform, SiToml, SiTurbo, SiTypescript, SiVercel, SiVim, SiVite,
  SiVitepress, SiVitest, SiVuedotjs, SiWebpack, SiYaml, SiYarn, SiZig
} from 'react-icons/si'
import {
...
export function colored(entry: IconEntry, size = 14): ReactNode {
...
export const FILE_ICON_BY_EXT: Partial<Record<string, IconEntry>> = {
...
export const FILE_ICON_BY_NAME: Partial<Record<string, IconEntry>> = {
  // Package managers
